...
const CartClient = () => {
  ...
  if (!cartPrdcts || cartPrdcts.length == 0) {
    ...
    let cartProductsTotal= cartPrdcts.reduce((acc:any,item:CardProductProps) => acc+item.quantity*item.price,0);
  return (
          ...
          {cartPrdcts.map((cart) => (
            <div className='flex items-center justify-between text-center my-5' key={cart.id}>
                  <div className='w-1/5 flex items-center justify-center '>
                      <Image src={cart.image} width={45} height={45} alt=''/>
              </div>
              ...
              <div className='w-1/5 text-orange-600 text-lg'>{cart.price}</div>
                  <div className='w-1/5'>
                      <Button text='Ürünü Kaldır' small onClick={()=> removeFromCart(cart)}/>
              </div>
            </div>
          ))}
              </div>
